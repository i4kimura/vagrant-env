...
gcc_build_dir = "#{Chef::Config[:file_cache_path]}/gcc-5.3.0/build/"
newlib_build_dir = "#{Chef::Config[:file_cache_path]}/newlib-2.2.0.20151023/build/"
...
remote_file "#{Chef::Config[:file_cache_path]}/gcc-5.3.0.tar.gz" do
  source "http://ftp.tsukuba.wide.ad.jp/software/gcc/releases/gcc-5.3.0/gcc-5.3.0.tar.gz"
end

execute "extract gcc" do
  cwd Chef::Config[:file_cache_path]
  command "tar xfz ./gcc-5.3.0.tar.gz"
  action :run
end
...
remote_file "#{Chef::Config[:file_cache_path]}/newlib-2.2.0.20151023.tar.gz" do
  source "ftp://sourceware.org/pub/newlib/newlib-2.2.0.20151023.tar.gz"
end

execute "extract newlib" do
  cwd Chef::Config[:file_cache_path]
  command "tar xfz ./newlib-2.2.0.20151023.tar.gz"
  action :run
end
